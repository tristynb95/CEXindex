...
// Reset restores View to Bakeries and Period to Last 3 Months, so those are the
...
test('the dashboard defaults Reset restores are the defaults the badge measures', () => {
  ...
  assert.match(html, /<option value="3" selected>Last 3 Months<\/option>/);
  assert.match(app, /state\.dashboardView = 'bakeries';\s*var dashboardViewSelect/);
  assert.match(app, /rollingWindow\.value = '3';/);
});
...
test('View and Period count towards the filter badge once they leave their defaults', () => {
  ...
  assert.match(app, /function isNonDefaultPeriod\(\)[\s\S]*?return rollingWindow\.value !== '3';/);
...
test('a pinned month counts as one filter, not two', () => {
  ...
  assert.match(app, /if \(monthSelect\.value\) return true;\s*return rollingWindow\.value !== '3';/);
});
...
test('Reset defaults the parked period rather than the one the Focus map holds', () => {
  assert.match(app, /if \(focusMapPeriodMemo\) \{[\s\S]*?focusMapPeriodMemo = \{ month: '', rolling: '3' \};\s*\} else \{/);
});
